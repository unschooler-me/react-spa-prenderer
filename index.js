...
async function getHTMLfromPuppeteerPage(browser, pageUrl, options) {
  try {
    ...
    page.waitForTimeout(options.delay);
 ...
 */
async function runPuppeteer(baseUrl, routes, dir, engine, options) {
  const browser = await puppeteer.launch(engine.launchOptions);
  for (let i = 0; i < routes.length; i++) {
    try {
      console.log(`Processing route "${routes[i]}"`);
      const html = await getHTMLfromPuppeteerPage(browser, `${baseUrl}${routes[i]}`, options);
...
async function run() {
  ...
  await runPuppeteer(staticServerURL, options.routes, options.buildDirectory, options.engine, options);
